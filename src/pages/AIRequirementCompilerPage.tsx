...
export default function AIRequirementCompilerPage() {
  ...
  const [state, setState] = useState<'idle'|'uploading'|'generating'|'clarification-required'|'oversized-warning'|'failed'>('idle')
  ...
  const handleConfirmClarifications = async () => {
    ...
    try {
      const qaList = clarifications.map((q, idx) => ({
        question: q,
        answer: clarAnswers[idx] || '',
      }))
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Transform raw notes into structured GitHub Copilot Agent prompts.</p>
      </div>
        ...
        <div className="flex flex-col h-full min-h-0 overflow-hidden">
          ...
          {state === 'oversized-warning' && (
            <Card className="p-4 border-yellow-200 bg-yellow-50">
              ...
              <p className="text-sm text-yellow-800 mt-2">Split this into smaller, focused requirements for deterministic prompts. Consider separating by feature, module, or phase.</p>
              <div className="mt-4 flex gap-2">
                <Button variant="outline" size="sm" onClick={() => {
                  setState('idle')
                  setOutput('')
                }}>Edit & Reduce Scope</Button>
                <Button size="sm" variant="secondary" onClick={() => {
                  setState('generating')
                  compileRequirements().then(result => {
                    setOutput(result)
                    setState('idle')
                  }).catch(() => setState('failed'))
                }} disabled={aiLoading}>Generate Anyway</Button>
              </div>
            </Card>
          )}
          ...
          {state === 'failed' && (
            <Card className="p-4 border-red-200 bg-red-50">
              ...
              <p className="text-sm text-red-700 mt-1">{aiError || 'Unknown error occurred. Please try again.'}</p>
          ...
          <PromptOutputPanel generating={state === 'generating'} output={output} />
        </div>
      </div>
    </div>
  )
